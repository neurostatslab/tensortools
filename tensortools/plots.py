...
def plot_factors(factors, figsize=None, plots='line', ylim='link', fig=None,
                 axes=None, yticks=True, width_ratios=None, scatter_kw=dict(),
                 line_kw=dict(), bar_kw=dict()):
    """Plots a KTensor.
    ...
    for sckw in scatter_kw:
        if "edgecolor" not in sckw.keys():
            sckw["edgecolor"] = "none"
        if "s" not in sckw.keys():
            sckw["s"] = 10

    # setup subplots (unless already specified)
    if fig is None and axes is None:
        grid_kw = dict(width_ratios=width_ratios)
        fig, axes = plt.subplots(
            rank, ndim, figsize=figsize, gridspec_kw=grid_kw)
        if rank == 1:
            axes = axes[None, :]
    ...
    # Adjust y-axis limits for each column of plots.
    for i, yl in enumerate(ylim):

        # Leave y-axis limits same as is.
        if yl is None:
            continue

        # Share y-axis limits among all plots in this column.
        elif yl == 'link':
            yl = [a.get_ylim() for a in axes[:, i]]
            y0, y1 = min([y[0] for y in yl]), max([y[1] for y in yl])
            [a.set_ylim((y0, y1)) for a in axes[:, i]]

        # Set each y-axis limits tightly.
        elif yl == 'tight':
            for r, a in in enumerate(axes[:, i]):
                a.set_ylim(np.min(factors[i][:, r]), np.max(factors[i][:, r]))

        # Set y-axis limits to user-defined limits.
        else:
            try:
                [a.set_ylim(yl) for a in axes[:, i]]
            except (ValueError, AttributeError):
                raise ValueError("Y-axis limits for mode-{} not properly "
                                 "specified. Expected string (either 'link' "
                                 "or 'tight') tuple specifying bounds but "
                                 "saw {}.".format(i, yl))
...
def plot_error(results, partition='train', ax=None, jitter=0.1, labels=True,
...
def plot_similarity(results, ax=None, jitter=0.1, labels=True,
                    scatter_kw=dict(), line_kw=dict()):
...
def plot_sim_v_err(results, axes=None, fig=None, figsize=None, labels=True,
                   sharex=True, sharey=True, format_axes=True,
                   scatter_kw=dict(edgecolor='none', color='k')):
    ...
    if fig is None and axes is None:
        ...
        if figsize is None:
            figsize = (2*n, 2*m)
        fig, axes = plt.subplots(
            m, n, figsize=figsize, sharex=sharex, sharey=sharey)

    # Use figure and axes passed in by the user.
    elif fig is None or axes is None:
        raise ValueError("If either 'fig' or 'axes' are given as keyword "
                         "arguments, both must be specifed.")
